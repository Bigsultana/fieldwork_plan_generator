...
function addRect(pptx, slide, left, top, width, height, options = {}) {
  slide.addShape(pptx.ShapeType.rect, {
    ...
    w: mm(width),
    h: mm(height),
    fill: options.fill ? { color: options.fill } : transparentFill(),
    line: {
...
function drawTitleBlock(pptx, slide, project, sheet, logo) {
  ...
  [projectL, drawingL, metaL].forEach((x) => {
    slide.addShape(pptx.ShapeType.line, {
      ...
      w: 0,
      h: mm(height),
      line: { color: COLOURS.black, width: 0.4 },
    });
  });
  ...
  addText(slide, project.companyName || "COMPANY NAME", companyTextL, top + 3, companyTextW, 13, {
    fontSize: 13,
    bold: true,
    color: accent,
  });
  addText(slide, project.companyAddress, companyTextL, top + 17, companyTextW, 15, {
    fontSize: 7.2,
    valign: "top",
  });
  const contact = [project.companyPhone, project.companyEmail, project.companyWebsite]
    .filter(Boolean)
    .join("  |  ");
  addText(slide, contact, companyTextL, top + 33, companyTextW, 12, {
    fontSize: 6.5,
  });

  addText(slide, "PROJECT", projectL + 1, top + 1, projectW - 2, 4, {
    fontSize: 6,
    color: COLOURS.midGrey,
    valign: "top",
  });
  addText(slide, project.projectTitle || "PROJECT TITLE", projectL + 1, top + 5, projectW - 2, 15, {
    fontSize: 12,
    bold: true,
    color: accent,
  });
  ...
  addText(slide, "DRAWING TITLE", drawingL + 1, top + 1, drawingW - 2, 4, {
    fontSize: 6,
    color: COLOURS.midGrey,
    valign: "top",
  });
  addText(slide, sheet.drawingTitle1 || "DRAWING TITLE", drawingL + 1, top + 5, drawingW - 2, 18, {
    fontSize: 14,
    bold: true,
  });
  addText(slide, sheet.drawingTitle2, drawingL + 1, top + 23, drawingW - 2, 12, {
    fontSize: 9.5,
    bold: true,
  });
  addText(slide, sheet.drawingTitle3, drawingL + 1, top + 35, drawingW - 2, 10, {
    fontSize: 8,
  });
...
export function createPresentation(project, preparedSheets, logo = null) {
  ...
  preparedSheets.forEach((sheet) => {
    ...
    if (sheet.image?.data && sheet.image.width > 0 && sheet.image.height > 0) {
      ...
      drawMissingPlaceholder(pptx, slide, sheet, sheet.imageError || "Unable to read image");
    }
  });
  return pptx;
}
...
export async function generatePresentation(project, sheets, logoFile, onProgress = () => {}) {
  const preparedSheets = await prepareSheets(sheets, onProgress);
  const logo = await prepareLogo(logoFile);
  onProgress(sheets.length, sheets.length, "Building PowerPoint");
  const pptx = createPresentation(project, preparedSheets, logo);
  const buffer = await pptx.write({ outputType: "arraybuffer", compression: true });
  return {
    buffer,
    missing: preparedSheets.filter((sheet) => !sheet.image),
  };
}
